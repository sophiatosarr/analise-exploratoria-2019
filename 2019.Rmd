...
Sku_status_dataset
- cod_prod: Código identificador do produto (Tipo: numérico)
- data_inicio: Data de início da validade do status do produto (Tipo:
character)
- data_fim: Data de fim da validade do status do produto (Tipo:
character)
...
character)
- data_fim: Data de fim da validade do preço do produto (Tipo:
character)
- preco: Preço do produto (Tipo: numérico)
...
character)
- data_fim: Data de fim da validade do custo do produto (Tipo:
character)
- custo: Custo do produto (Tipo: numérico)

Employee_final
- id_employe: Código identificador do funcionário (Tipo: numérico)
...
character)
 - end_date: Data de término do contrato do funcionário (Tipo:
character)
- store_id: Código identificador da loja onde o funcionário trabalha
(Tipo: character)
...
labs(title = "Distribuição da Meta de Vendas", x = "Meta de Vendas", y =
"Frequência")
...
```{r}
ggplot(sku_price, aes(x = preco)) + geom_density(fill = "blue", alpha =
0.5) + labs(title = "Densidade do Preço dos Produtos", x = "Preço", y =
"Densidade")
```
...
ggplot(sku_price, aes(y = preco)) + geom_boxplot(fill = "orange") +
...
```{r}
ggplot(sku_cost, aes(y = custo)) + geom_boxplot(fill = "purple") +
